...
#: 随机臂口径 v4（P79 / D4）：「可成交化」时 `S = ∅`（抽出来的每只都不到一手）
#: 允许**重抽几次**（只重抽 exposure，其余不动，用同一个 rng 序列继续）。
#: 上限存在的理由与其它预算常量一致：不给一个无上限的重试 —— 那会让「同一个种子
#: 跑多久」变成一个外部条件。8 次仍为 ∅ ⇒ 退到「一手最便宜的池内标的」。
RANDOM_RETRY_MAX: int = 8
