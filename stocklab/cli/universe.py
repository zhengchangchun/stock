...
def _sync(conn: sqlite3.Connection, universe, *, now: str) -> int:
    ...
    repo.log_event(
        ...
        f"新增 instruments {len(added)} 行、补空 sector {len(sector_filled)} 行"
...
def _doctor(conn: sqlite3.Connection, universe) -> tuple[list[str], list[str]]:
    ...
    if not conn.execute(
        ...
        instr: dict[str, str | None] = {}
    else:
        instr = {str(r["code"]): r["sector"] for r in
                 conn.execute("SELECT code, sector FROM instruments")}
    ...
    have_sector = sum(1 for c in universe.codes if str(instr.get(c) or "").strip())
