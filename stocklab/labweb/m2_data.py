...
#: 错判案例集的取数与三个常量**住在 `m2/cases.py`**（P58 §3 搬出去的那一份）。
#: 这里只是把公开名字转出来：页面与报告一直读的就是同一段代码，口径仍然只有一份。
CASE_LIMIT: int = m2_cases.CASE_LIMIT
ATTR_FIELD: str = m2_cases.ATTR_FIELD
ATTRIBUTION_NOTE: str = m2_cases.ATTRIBUTION_NOTE
miss_cases = m2_cases.miss_cases
...
# ---------- ③ 错判案例集 ----------
# 取数函数已搬到 `stocklab/m2/cases.py`（P58 §3），本文件的 `miss_cases` 是它的
# 转出（见文件头部的 import 区），调用方（页面、`_cycle_cases`、插桩5）读同一份口径。
# 下面这一节保留标题，是为了让「① ② ③」的编号与页面/报告的小节号继续对得上。
