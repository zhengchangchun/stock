...
| L4 | F2 的「该状态的无条件频率」 | **首跑后修订**：`Δ = 延续率 − 该状态在本段的**边际频率**`（预注册原文），CI 由日级延续率按日聚类 bootstrap 后整体平移；当日配对版本降级为**仅登记**的 `delta_day_matched` | 首跑误用「当日行占比」当基线 → 选择效应把基线抬高（出现 UP 的那些天本身就是 UP 占比高的日子），UP 侧 Δ 被压成 +0.64%（ERROR_DIARY #32）。预注册原文说的是边际频率，**以原文为准** |
