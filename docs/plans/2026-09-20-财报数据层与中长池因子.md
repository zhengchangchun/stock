...
- Create: `stocklab/data/notice_date.py`
...
Expected: FAIL —— `ModuleNotFoundError: No module named 'stocklab.data.notice_date'`

- [ ] **Step 3: 实现**

新建 `stocklab/data/notice_date.py`：
...
```bash
git add stocklab/data/notice_date.py tests/test_notice_date.py
...
> **依赖方向**：`notice_date.py` 放 `stocklab/data/`（不是 `candidate/`）——它是
> 「源数据语义」，`data/fetch.py` 与 `candidate/indicators.py` 都要用。这样依赖方向是
> `candidate/ → data/`，与既有分层一致；反过来（`data/` import `candidate/`）会让
> 最底层依赖业务层，属架构倒置。
