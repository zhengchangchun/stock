...
**新建 `stocklab/config/replay.py`**（**中性位置** —— controller 预检时裁决：放任一侧都会让 `candidate/` 与 `plugin/` 互相 import）：

```python
"""回放的**共享常量**。没有逻辑，只有常量。"""

#: 各池的调仓周期（交易日）。**固定常量** —— 它是回放的**规模定义**，不是
#: 可调参数。允许调它等于允许「改下游提分」，归因立刻失效（单变量原则）。
REBALANCE_DAYS: dict[str, int] = {"short": 5, "mid": 20, "long": 60}
```

`candidate/replay.py` 的文件头加：

```python
from stocklab.config.replay import REBALANCE_DAYS
```

（**不要**写 `from stocklab.plugin.sandbox import REBALANCE_DAYS` —— 那是预检里裁决掉的写法。）
...
- Produces:
  ...
  - **`SandboxDeps`**（frozen dataclass）：`replay` / `benchmark_excess` / `rebalance_marks` 三个 callable（后两个可 `None`）
  - `run_sandbox(conn, *, candidate_script_id, baseline_script_id, pool, window_start, window_end, now, deps: SandboxDeps | None = None) -> SandboxVerdict`
  - `REBALANCE_DAYS` 改为 `from stocklab.config.replay import REBALANCE_DAYS`（不再在本模块定义）

> **controller 预检裁决**：三个注入 callable **收成一个 `SandboxDeps` dataclass**，`run_sandbox` 只加**一个** `deps` 参数。`deps is None` → 保持 fail-closed。这样签名稳定（将来加第四个注入不用改签名），也不会让 `run_sandbox` 的参数表变成注入清单。
...
- Produces:
  ...
  - `replay.rebalance_marks(conn, *, pool, start, end) -> list[str]`
  - `SandboxVerdict.detail["candidate_excess_index300"]` / `["baseline_excess_index300"]` / `["scripts"]`

> **本任务只用 Task 5 已经建好的 `SandboxDeps`** —— 把 `benchmark_excess` 与 `rebalance_marks` 填进那个 dataclass，**不要再往 `run_sandbox` 的签名上挂新参数**（controller 预检已裁决收成一个 `deps`）。
...
**3a-pre.** `stocklab/candidate/replay.py` 需要这两样：
...
（`REBALANCE_DAYS` 已在 Task 4 里挪到 `stocklab/config/replay.py`，本文件头已 import 它。）

**3a.** `stocklab/candidate/replay.py` 追加：
...
def rebalance_marks(conn: sqlite3.Connection, *, pool: str, start: str,
    ...
    """窗口内的调仓日序列。供 `sandbox` 经 `SandboxDeps` 注入调用
    （`plugin/` 不得 import 本模块）。"""
    days = _trading_days(conn, start, end)
    return rebalance_dates(days, period=REBALANCE_DAYS[pool],
                           start=start, end=end)
```

并在文件头把 `REBALANCE_DAYS` 的来源改为：

```python
from stocklab.config.replay import REBALANCE_DAYS
```

（**不是** `from stocklab.plugin.sandbox import REBALANCE_DAYS` —— 那是 controller 在预检里裁决掉的写法。）
...
class _FakeClient:
    ...
    def get_text(self, url, *, headers, source, cache_key):
        ...
        i = len(self.calls) - 1
        payload = self.pages[i] if i < len(self.pages) else {"data": {"sz000333": {"qfqday": []}}}
        return json.dumps(payload)


def _page(rows):
    return {"data": {"sz000333": {"qfqday": rows}}}


def _row(date: str, close: float = 10.0):
    """腾讯 day 行：[date, open, close, high, low, volume]。"""
...
> 具体的解析入口以 `data/sources/tencent.py` 的实际形状为准；实现时按它调整 `_page` / `_row` 的 payload 形状，并在报告里写明你改成了什么。
