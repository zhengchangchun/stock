# 实验：sigma-rv-pct（自身 20 日 RV 分位条件化 sigma，**已收尾 · `falsified`**）
...
|---|---|---|
| 方向（越大越好） | **-0.0108 ± 0.0109** [-0.0321, +0.0106] | 651 |
| Brier（越小越好） | **+0.0150 ± 0.0040** [+0.0071, +0.0229] | 651 |

配对计数：`n_pairs=1302`、仅基线有 0、任一侧不可评分 0
（变体行 4689 vs 基线行 4898：差额 = `asof` 处不足 270 根
bar、RV 分位算不出的日子，已**显式计数**而非填 0）。train 段 gate = `FLAT`。

**gate = `LOSE`**（651 有效交易日，门槛 120）：

- 方向 Δ=-0.0108 ± 0.0109，95% CI [-0.0321, +0.0106] → 未达显著变好
- Brier Δ=+0.0150 ± 0.0040，95% CI [+0.0071, +0.0229]（越小越好）→ 未达显著变好
...
即使 WIN 也封存）。决策行已写入 `experiment_decisions`（`variant_id=sigma-rv-pct`，`split=train/validate`，
`metric=direction/brier`，`gate_status=LOSE`）。

**学到了什么**：差得最多：三个变体里 Δbrier 最正（+0.0150）、方向也最负。**自身 RV 分位并不携带 60 日窗丢掉的可用状态信息**（至少以「缩放到 ±50%」这种形式不携带）。
...
- **唯一变量**：`sigma_mode = rv_pct`（`mu_mode` / 窗口 / 标签带 / 标的集合 / 样本区间逐行未动）
- **结论**：validate 651 日 方向 Δ=-0.0108 ± 0.0109 [-0.0321, +0.0106]、Brier Δ=+0.0150 ± 0.0040 [+0.0071, +0.0229] → **`falsified`**，基线 `pit-rw-v1.0.1` 保持不表
