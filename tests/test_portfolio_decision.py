...
def test_lines_come_from_discipline_not_a_second_copy():
    ...
    d = _decide(86.0)
    assert d["lines"]["stop_loss_close"] == LINES["stop_loss_close"] == 82.14
...
@pytest.mark.parametrize("close,expected", [
    (82.13, STATE_STOP),   # 跌破止损线
    (82.14, STATE_HOLD),   # **正好落线上不算破**（与 check_stop_loss_close 同口径）
...
def test_close_at_stop_line_is_not_a_breach():
    """82.14 是「落在线上」，不是「跌破」—— 差一分才破。"""
    assert _decide(82.14)["state"] == STATE_HOLD
    assert _decide(82.13)["state"] == STATE_STOP


def test_headline_is_state_specific_and_human():
    assert _decide(86.0)["headline"] == "持有不动。"
    assert "不能补仓" in _decide(87.5)["headline"]
    assert "全清" in _decide(82.13)["headline"]


# ---------- 2. 全清试算：走 CostModel 权威入口，逐分一致 ----------

@pytest.mark.parametrize("close", [82.13, 82.14, 86.00, 87.00, 87.50, 100.00])
...
def test_sell_all_uses_stock_policy_not_etf():
    ...
    s = sell_to_cash(82.14, QTY)
    m = CostModel(asset_class=ASSET_STOCK)
    gross = 82.14 * QTY * (1 - m.slippage_bps / 10_000.0)
...
def test_sell_all_without_total_assets_is_none():
    ...
    d = _decide(82.13, total_assets=None, cash=None)
...
@pytest.mark.parametrize("close", [82.13, 86.00, 87.50])
...
def test_price_does_not_affect_state():
    """同样的收盘价，盘中价怎么变都不改结论。"""
    a = _decide(82.13, price=99.0)
    b = _decide(82.13, price=1.0)
